...
(function (global) {
    ...
    TextHighlighter.createWrapper = function (options) {
        ...
        span.setAttribute('data-bs-content', mobile);
      ...
      let onSpeechToAction = function (event, action, entities) {
        ...
        if (action === "START" || action === "CURRENT")
          ...
          switch (action) {
            ...
            case "CHECK": $(self).find('.btn-check-answer').trigger('click'); break;
            case "NONE":
            default:
            ...
            let onLanguageLoaded = function (err, i18n) {
                ...
                let buildMobileTableOptions = function () {
                    ...
                    let $rows = $dragOptionsList.find('.drag-item').map(function () {
                        ...
                        let $cell = $('<div class="d-grid" />')
                        let $button = $('<button />')
                            .addClass('btn btn-success mb-2 btn-option')
                    ...
                    let $checkAnswerButton = $table.find('.btn-check-answer')
                    $checkAnswerButton
                ...
                let playReader = async function ({ action, message, onEnd, alertType } = {}) {
                    ...
                    await new Promise((resolve) => {
                        if (alertType == "screen-reader") {
                            ...
                            setTimeout(function () {
                                $('#drag-drop-message-' + widgetId).html('<span class="visually-hidden">' + message + '</span>');
                                resolve();
                                setTimeout(function () {
                ...
                let buildAccessibleDragDropModal = function () {
                    ...
                    let $options = $self.find('.drag-description').map(function () {
                        let $cell = $('<li />').addClass('m-1 modal-option d-grid');
                        let $button = $('<button />')
                ...
                let onSpeechToAction = function (event, action, entities) {
                    ...
                    if (["CURRENT", "START", "NEXT", "PREVIOUS", "GOTO"].includes(action)) {
                        ...
                        $(".modal.show").length && modal && modal.hide();
                        $(".alert").length && $('.alert').alert('close');
                    }
...
(function () {
    ...
    // Set highlighting colour
    $(document).on('click', '.highlight-color', function() {

        $('.highlight-color').removeClass('selected').attr('aria-pressed', false);
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.couples').each(function () {
            ...
            let onSpeechToAction = function (event, action, entities) {
                ...
                if (action === "START" || action === "CURRENT")
                    ...
                    switch (action) {
                        ...
                        case "CHECK": $(self).closest('.widget-couples').find('.btn-check-answer').trigger('click'); break;
            ...
            let items = couples.map((couple, idx) => {
                ...
                let $state = $("<span />").addClass('couple-state visually-hidden').attr('id', widgetId + "-state-" + idx).html("")
                let $prefix = $("<span />").addClass('couple-prefix visually-hidden').attr('id', widgetId + "-prefix-" + idx).html(i18n("accessibility-option") + " " + (idx + 1) + " " + i18n(couple.type === "img" ? "common-image" : "common-text"))
            ...
            if (!gamification) {
                ...
                $(this).closest('.widget-couples').on('click', '.btn-check-answer', checkGamificationCouples);
            }
        })
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.container-test').each(function () {
            ...
            let onSpeechToAction = function (event, action, entities) {
                ...
                let $checkAnswerBtn = $(self).find('.btn-check-answer')
        ...
        $('.container-test').on('click', '.btn-reset', function (e) {
            ...
            $widget.find('input:checked').prop('checked', false);
            $widget.find('.iconresult').removeAttr('aria-label').empty();
        })

        $('.container-test').on('click', '.btn-check-answer', function (e) {
            let gamification = $('body').hasClass('gamification');
            let $widget = $(this).closest('.container-test');
            let id = $widget.attr('id');
            let $questions = $widget.find('.question');
            ...
            $questions.each(function () {
                ...
                let $question = $(this);
                let allCorrect;
                // Set answers and which ones are correct
                if ($question.is(".multiple")) {
                    // All checkboxes are answers, even if unchecked
                    $answers = $question.find('input');    
                    $correctAnswers = $answers.filter(function () { return $(this).prop("checked").toString() === $(this).val() });
                    allCorrect = ($correctAnswers.length === $answers.length);
                    score += ($correctAnswers.length / $answers.length);
                    correctQuestions += allCorrect;
                } else {
                    // Only checked radiobuttons are answers
                    $answers = $question.find('input:radio:checked');
                    $correctAnswers = $answers.filter(function () { return $(this).val() === "true" });
                    allCorrect = $correctAnswers.length === 1;
                    score += allCorrect;
                    correctQuestions += allCorrect;
                }
            ...
            $(this).trigger('gamificate', [correctQuestions === numQuestions, score / numQuestions]);
            if (!gamification) {
                // Show the user immediate global feedback
                $widget.find('.resultado').html(i18n("TextNumberCorrect", { num: correctQuestions }));
                if (numQuestions === correctQuestions) {
                    setObjetivoCompleto(`objetivo${id}`, "«widget.name»", "«widget.name»");
                }
            }
        });

        $('.container-test input').on('change', function (e) {
...
(function () {
    ...
    if (typeof annyang !== 'undefined' && annyang) {
        ...
        let onLanguageLoaded = function (err, i18n) {
            ...
            $("#speech-to-action-global").parent().removeClass('d-none');
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.widget-missing-words').each(function () {
            ...
            $(this).on('click', '.btn-check-answer', function () {
                let gamification = $('body').hasClass('gamification');
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.widget-correct-word').each(function () {
            ...
            $widget.on('click', '.btn-check-answer', function () {
                let gamification = $('body').hasClass('gamification');
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.widget-hangman').each(function () {
            ...
            $widget.on('submit', function (e) {
                ...
                setTimeout(function () {
                    $newLetter.prepend('<span class="visually-hidden">' + (positions.length ? i18n("hangman-correct-letter") : i18n("hangman-wrong-letter")) + '</span>')
                }, 50);
            ...
            $widget.on('click', '.btn-reset', function () {
                ...
                $widget.find('.letter').filter(function (idx) { return solution[idx] !== ' '}).html(`
					<span class="visually-hidden">`+i18n("hangman-blank")+`</span>
            ...
            let onSpeechToAction = function (event, action, entities) {
                ...
                if (action === "CLOSE" && ($(".modal.show").length || $(".alert").length)) {
                    ...
                    $widget.find('.btn-check-answer').trigger('click');
                }
                else {
                    switch (action) {
                        case "RESET": $widget.find('.btn-reset').trigger('click'); break;
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        let init = function (focusCoordinates) {
            ...
            let btnsArr = words.map((word, idx) => {
                let $status = $('<span />').attr('id', 'word-item-status-' + id + '-' + idx).addClass('status visually-hidden');
                ...
                if (action === "CLOSE" && ($(".modal.show").length || $(".alert").length)) {
                    ...
                    switch (action) {
                        case "SELECT":
                            ...
                            break;
                        case "CHECK": $container.find('.btn-check-answer').trigger('click'); break;
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        let init = function () {
            ...
            $widget.on('keydown', '.dropdown .btn[data-bs-toggle="dropdown"]', onKeyDownButton);
                ...
                if (action === "CLOSE" && ($(".modal.show").length || $(".alert").length)) {
                    ...
                    switch (action) {
                        case "SELECT":
                            ...
                            break;
                        case "CHECK": $widget.closest('.content-element').find('.btn-check-answer').trigger('click'); break;
            ...
            $widget.closest('.content-element').on('click', '.btn-check-answer', function () {
...
(function () {
    ...
    let playReader = async function (id, message) {
        ...
        await new Promise((resolve) => {
            ...
            $container.text("");
            setTimeout(function () {
                let $msg = $('<span />').addClass('visually-hidden').text(message);
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.widget-puzzle').each(function () {
            ...
            tmpImage.onload = function () {
                ...
                $widget.find('[data-bs-toggle]').dropdown();
                ...
                $widget.on('keydown', '.dropdown-menu', onKeyDownDropdown);
                $widget.on('keydown', '.dropdown .piece[data-bs-toggle="dropdown"]', onKeyDownInput);
...
(async function () {
    ...
    // Trusted domains to allow cross origin requests
    const ALLOWED_HOSTS = [
        "opencontent.upct.es",
        "contentserver.upct.es"
    ];
    // Whitelist of valid origin parameters
    const ALLOWED_ORIGIN_PARAMETERS = ["indieopen", "upctforma"];

    // Get the author and unit of the referer unit
    const params = new URLSearchParams(window.location.search);
    const paramReferer = params.get('ref');
    // Get the origin query parameter, we will need later on to build the URL of the related units
    ...
    if (paramReferer) {
        try {
            ...
            if (parsed && 
                (parsed.hostname === window.location.hostname || ALLOWED_HOSTS.includes(parsed.hostname))) {
    ...
    const parseRelatedUnit = (unit, level = 3) => {
        ...
        let current, url, originQueryParam = "";
        ...
        if (unit.url === '#')
            ...
            // Only replace hostname if the target url is opencontent or contentserver
            if (ALLOWED_HOSTS.includes(hostname)) {
                switch (window.location.hostname) {
                    // Replace the hostname of the target unit with the current one
                    case 'opencontent.upct.es':
                    case 'contentserver.upct.es':
                        hostname = window.location.hostname;
                        break;
                }
                // Add a origin query parameter to the target url
        ...
        return `<li>
            <h${level} class="related-entry related-unit">
                <a class="nav-element ${current ? "current" : ""}" 
                  href="${ current ? '#' : (url + "?ref="+referer + originQueryParam) }"
    ...
    const offcanvas = bootstrap.Offcanvas.getOrCreateInstance(tableContentsOffcanvas);
